...
def test_gd_opt_mlp():
    ...
    # training
    if TEST_ALGO == "Momentum":
        losses = Momentum_opt_MLP(
            L_func=L_func,
            dL_func=dL_func,
            mlp=mlp,
            xs=X_train,
            ...
            batch_size=32,      # 批大小
            N_I=10,             # 迭代次数
            beta=0.9,           # Momentum 一阶系数
            alpha_0=0.001,       # 初始学习率
            gamma=0.99,         # 学习率衰减率
        )
    elif TEST_ALGO == "RMSProp":
        losses = RMSProp_opt_MLP(
            L_func=L_func,
            dL_func=dL_func,
            mlp=mlp,
            xs=X_train,
        ...
        losses = Adam_opt_MLP(
            ...
            batch_size=32,      # 批大小
            N_I=10,             # 迭代次数
            beta_1=0.9,         # Adam 一阶系数
            beta_2=0.99,        # Adam 二阶系数
            alpha_0=0.001,       # 初始学习率
            gamma=0.99,         # 学习率衰减率
        )
